...
class LTSRGBOnlyDetector(BaseOODDetector):
    """LTS (Large-scale Temperature Scaling) using RGB Modality Only"""
    ...
    def lts_scale(self, x, percentile=None):
        ...
        logging.info("  ⚡ Computing LTS scale (RGB Only):")
    ...
    def compute_scores_with_features(self, logits, individual_features):
        """
        Main method: Compute LTS scores using RGB modality only
        Args:
        ...
        """
        logging.info("🚀 [LTS_RGB_Only] Starting computation with RGB features only")
        ...
        rgb_norm = torch.mean(torch.norm(rgb_features_normalized, p=2, dim=1)).item()
        
        logging.info(f"  ✨ RGB feature magnitude: {rgb_magnitude:.4f}")
        logging.info(f"  ✨ RGB feature L2 norm: {rgb_norm:.4f}")
        
        # Step 2: Compute LTS scale from RGB features
        lts_scale = self.lts_scale(rgb_features_normalized)
        
        # 🔍 DEBUG: LTS_RGB_Only scale statistics
        logging.info(f"  🔍 [DEBUG] LTS_RGB_Only scale - Mean: {lts_scale.mean().item():.6f}, Std: {lts_scale.std().item():.6f}")
        logging.info(f"  🔍 [DEBUG] RGB features - Mean: {rgb_features_normalized.mean().item():.6f}, Std: {rgb_features_normalized.std().item():.6f}")
        ...
        logging.info(f"  🎯 Final OOD scores (RGB Only) - Avg: {avg_score:.4f}, Min: {min_score:.4f}, Max: {max_score:.4f}")
        logging.info("✅ [LTS_RGB_Only] Computation completed")
