...
class LTSGyroOnlyDetector(BaseOODDetector):
    """LTS (Large-scale Temperature Scaling) using Gyro Modality Only"""
    ...
    def lts_scale(self, x, percentile=None):
        """
        Compute LTS scale for Gyro features only
        """
        ...
        logging.info("  ⚡ Computing LTS scale (Gyro Only):")
    ...
    def compute_scores_with_features(self, logits, individual_features):
        """
        Main method: Compute LTS scores using Gyro modality only
        Args:
            logits: (batch_size, num_classes) - pre-computed logits
            individual_features: list of feature tensors for each modality
                                [RGB, Gyro, Acce, ...] - Gyro is at index 1
        Returns:
            scores: numpy array of shape (batch_size,)
        """
        logging.info("🚀 [LTS_Gyro_Only] Starting computation with Gyro features only")
        ...
        # Gyro features 통계 로깅
        gyro_magnitude = torch.mean(torch.abs(gyro_features)).item()
        gyro_norm = torch.mean(torch.norm(gyro_features_normalized, p=2, dim=1)).item()
        
        logging.info(f"  ✨ Gyro feature magnitude: {gyro_magnitude:.4f}")
        logging.info(f"  ✨ Gyro feature L2 norm: {gyro_norm:.4f}")
        
        # Step 2: Compute LTS scale from Gyro features
        lts_scale = self.lts_scale(gyro_features_normalized)
        
        # 🔍 DEBUG: LTS_Gyro_Only scale statistics
        logging.info(f"  🔍 [DEBUG] LTS_Gyro_Only scale - Mean: {lts_scale.mean().item():.6f}, Std: {lts_scale.std().item():.6f}")
        logging.info(f"  🔍 [DEBUG] Gyro features - Mean: {gyro_features_normalized.mean().item():.6f}, Std: {gyro_features_normalized.std().item():.6f}")
        ...
        logging.info(f"  🎯 Final OOD scores (Gyro Only) - Avg: {avg_score:.4f}, Min: {min_score:.4f}, Max: {max_score:.4f}")
        logging.info("✅ [LTS_Gyro_Only] Computation completed")
