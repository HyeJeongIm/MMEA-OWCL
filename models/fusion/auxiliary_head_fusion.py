...
class AuxiliaryHeadFusion(nn.Module):
    ...
    def __init__(self, feature_dim, modality, dropout, num_classes=32, confidence_method="max_prob"):
        ...
        self.consensus = ConsensusModule('avg')  # 평균 consensus 사용
        self.num_segments = None  # TBNBaseline에서 설정됨
    ...
    def forward(self, features, targets=None):
        ...
        if len(self.modality) > 1:
            ...
            for modality_name, feature in modality_features.items():
             ...
             # 🎯 신뢰도 기반 가중치 정규화 (단순 합 정규화 - Winner-takes-all 강화)
            if confidences:
            ...
            # 🎯 Single modality에서도 auxiliary head 활용
            modality_name = self.modality[0]  # 첫 번째 (유일한) 모달리티
            feature = features[0]
            
            if modality_name in self.auxiliary_heads:
                # Auxiliary head로 신뢰도 계산
                aux_logits = self.auxiliary_heads[modality_name](feature)
                auxiliary_logits = {modality_name: aux_logits}
                confidence = self._compute_confidence(aux_logits)
                
                # 🎯 신뢰도를 직접 가중치로 사용 (절대적 스케일링)
                # 높은 신뢰도 → 특징 강화, 낮은 신뢰도 → 특징 약화
                weight = confidence.unsqueeze(1)  # [Batch, 1]
                weighted_feature = weight * feature  # 신뢰도 기반 스케일링
                
                x = self.dropout_layer(weighted_feature)
                weight_list = [confidence]  # 실제 신뢰도 값 저장
            else:
                # Fallback: Auxiliary head 없는 경우
                x = self.dropout_layer(feature)
                auxiliary_logits = {}
                weight_list = [torch.ones(x.size(0), device=x.device)]
