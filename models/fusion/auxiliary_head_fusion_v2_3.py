...
class AuxiliaryHeadFusionV2_3(nn.Module):
    ...
    def update_task(self, task_id):
        ...
        if task_id == 0:
            ...
            logging.info(f"🔥 Task {task_id}: Warm-up enabled (epochs 0-{self.warmup_epochs-1})")
        else:
            logging.info(f"🎯 Task {task_id}: No warm-up (auxiliary weights from start)")
    ...
    def forward(self, features, targets=None):
        ...
        if len(self.modality) > 1:
            ...
            if confidences:
                ...
                # 🔧 디버깅: 가중치 선택 과정
                if self.first_forward_per_task.get(self.current_task_id, False):
                    print(f"🔍 Weight Selection Debug (Task {self.current_task_id}):")
                    print(f"   🔥 Current Epoch: {self.current_epoch} (Warm-up until epoch {self.warmup_epochs})")
                    print(f"   🔥 Weight Type: {weight_type}")
                    print(f"   confidence_tensor: {confidence_tensor.shape}")
                    print(f"   final_weights: {weights[0].detach().cpu().numpy()}")
