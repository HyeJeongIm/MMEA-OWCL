...
class IMUEntropyGateFusion(nn.Module):
    ...
    def _compute_imu_weights(self, f_gyro, f_acce):
        ...
        핵심 로직:
        - 공통 특징 공간으로 투영 → 확률 분포로 변환 → 각각의 entropy 계산 → 상호 보완적 가중치 계산
        ...
        # 3. 적응적 가중치 계산
        # - 차이가 클 때: 낮은 엔트로피(확실한) 센서에 더 높은 가중치
        # - 차이가 작을 때: 평균 엔트로피에 따라 결정
        
        # Gyro 가중치: Acce가 불확실할수록 Gyro를 더 활용
        gyro_weight_factor = torch.where(
            entropy_diff > 0.1,  # 차이가 충분히 클 때
            1.0 - gyro_entropy_norm + 0.5 * acce_entropy_norm,  # 확실성 기반
            1.0 - 0.5 * avg_entropy  # 전체 불확실성 기반
        )
        
        # Acce 가중치: Gyro가 불확실할수록 Acce를 더 활용
        acce_weight_factor = torch.where(
            entropy_diff > 0.1,  # 차이가 충분히 클 때
            1.0 - acce_entropy_norm + 0.5 * gyro_entropy_norm,  # 확실성 기반
            1.0 - 0.5 * avg_entropy  # 전체 불확실성 기반
        )
