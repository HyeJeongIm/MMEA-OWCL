...
class AuxiliaryHeadFusionV2_9(nn.Module):
    """
    🔥 Auxiliary Head Pretrain 전략 기반 융합 모듈 (v2_9)
    
    핵심 개선사항 (vs v2_8):
    - ✅ Batch Normalization 추가: 융합된 특징에 BN 적용하여 학습 안정화
    
    이전 버전 대비 개선사항 (vs v2_6):
    ...
    3. ✅ Raw Confidence 기반 Fusion: confidence 값 자체를 정규화 없이 그대로 가중치로 사용
    ...
    - **Phase 2 (Epoch 5+)**: Frozen auxiliary head + raw confidence fusion
      * Auxiliary head 가중치 고정 (no gradient)
      * Raw confidence 값 자체를 가중치로 사용 (정규화 없음!)
      * Auxiliary head는 inference만 수행 (confidence 추출용)
    
    학습 목표:
    1. 각 task마다 pretrain으로 안정적인 auxiliary head 학습
    2. Raw confidence score를 통한 모달리티별 신뢰도 직접 반영
    3. 정규화 없이 confidence 값 자체로 가중치 부여
    """
    ...
    def __init__(self, feature_dim, modality, dropout, num_classes=32, 
        ...
        if len(self.modality) <= 1:
            raise ValueError("AuxiliaryHeadFusionV2_9 requires multiple modalities")
        
        input_dim = len(self.modality) * feature_dim

        # --- 🔧 수정된 부분 시작 ---
        # 배치 정규화 레이어 추가. fc1의 입력 차원(input_dim)과 동일하게 설정.
    ...
    def set_epoch(self, epoch):
        ...
        if epoch == self.pretrain_epochs and not self.auxiliary_heads_frozen:
            ...
            if epoch not in self.epoch_logged:
                ...
                logging.info(f"   Phase Transition: Pretrain → Raw Confidence Fusion")
                logging.info(f"   Weight Mode: Uniform (1:1:1) → Raw Confidence (no normalization)")
                logging.info(f"   Auxiliary Heads: Training → FROZEN (inference only)")
                logging.info(f"   From now on: Using raw confidence values as weights directly")
                logging.info(f"{'='*70}")
    ...
    def forward(self, features, targets=None):
        """
        Forward pass: Pretrain + Raw Confidence Fusion (모든 task에 적용)
        ...
        Phase 2 (Raw Confidence Fusion, epoch 5+ in each task):
        - Auxiliary heads frozen (inference only)
        - Raw confidence 값 자체를 가중치로 사용 (정규화 없음!)
        ...
        if len(self.modality) > 1:
            ...
            # 🎯 v2_9: Pretrain에서는 균등 가중치, 이후에는 raw confidence 값 자체를 가중치로 사용
            if confidences:
                confidence_tensor = torch.stack(list(confidences.values()), dim=1)
                
                if is_pretrain:
                    # Phase 1: Uniform weights (1:1:1, no scaling)
                    weights = torch.ones_like(confidence_tensor)
                    weight_type = "Pretrain (1:1:1 uniform)"
                    phase = "Pretrain"
                else:
                    # Phase 2: Raw confidence values as weights (NO normalization!)
                    weights = confidence_tensor
                    weight_type = "Raw Confidence (no normalization)"
                    phase = "Frozen Inference"
                
                # 🔧 디버깅: 첫 forward 또는 주요 전환 시점
                should_debug = (
                    self.first_forward_per_task.get(self.current_task_id, False) or
                    (self.current_task_id == 0 and self.current_epoch in [0, self.pretrain_epochs] and 
                ...
                # --- 🔧 수정된 부분 시작 ---
                # 이어 붙인 특징(x)을 fc1에 넣기 전에 배치 정규화 적용.
                x = self.bn1(x)
                # --- 🔧 수정된 부분 끝 ---
        ...
        if self.first_forward_per_task.get(self.current_task_id, False):
            ...
            logging.info(f"   🔧 Architecture: Pretrain + Raw Confidence Fusion Strategy")
            ...
            logging.info(f"   🔒 Current Phase: {'Pretrain' if is_pretrain else 'Frozen Inference (Raw Confidence)'}")
        ...
        return {
            ...
            'fusion_type': 'auxiliary_head_v2_9',
    ...
    def compute_total_loss(self, main_loss, auxiliary_loss=None):
        ...
        Raw confidence fusion phase (epoch 5+ in each task):
