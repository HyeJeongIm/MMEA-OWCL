...
class AuxiliaryHeadFusionV2_4(nn.Module):
    """
    🔥 Warm-up 지원 Auxiliary Head 기반 융합 모듈 (v2_4) - FIXED VERSION
    
    핵심 개선사항 (vs v2_3):
    1. ✅ Epoch 관리 방식 개선: set_epoch() 메서드로 외부에서 epoch 주입
    2. ✅ 배치 카운팅 기반 자동 epoch 감지 (fallback)
    3. ✅ Warmup 전환 시점 명확한 로깅
    4. ✅ Auxiliary loss 학습 추적 강화
    
    Warm-up 메커니즘:
    - 초기 W epoch (기본 5): 균등 가중치 (1:1:1) 사용
    ...
    def __init__(self, feature_dim, modality, dropout, num_classes=32, 
        ...
        Args:
            ...
            warmup_epochs: Warm-up epoch 수 (기본값: 5)
        """
        super().__init__()
