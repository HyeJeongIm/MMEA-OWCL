...
class IMUKLGateFusion(nn.Module):
    ...
    def _compute_imu_weights(self, f_gyro, f_acce):
        ...
        핵심 로직:
        - 공통 특징 공간으로 투영 → 확률 분포로 변환 → KL divergence 계산 → sigmoid로 게이트 강도 계산
        - KL divergence가 클수록 (분포가 다를수록) 두 센서를 모두 강하게 활용
        ...
        # 🔥 게이트 강도 계산: KL divergence가 클수록 강한 가중치
        # sigmoid를 사용해 min_weight ~ 1.0 범위로 매핑
        gate_strength = torch.sigmoid(self.alpha * kl_div)  # [Batch]
