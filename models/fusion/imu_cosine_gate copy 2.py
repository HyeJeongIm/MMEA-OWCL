...
class IMUCosineGateFusion(nn.Module):
    ...
    가중치 전략 (극단적 게이팅):
    - 코사인 유사도가 높을수록 (positive synergy): 두 센서를 거의 차단 (≈0.05)
    - 코사인 유사도가 낮을수록 (negative synergy): 두 센서를 강력하게 활용 (≈1.0)
    ...
    def _compute_imu_weights(self, f_gyro, f_acce):
        ...
        # 🔥 극단적 게이트 강도 계산: positive vs negative synergy 극대화!
        # bias_offset (-7.0)으로 완전히 비슷할 때 거의 차단
        # 비슷할 때 (difference≈0): sigmoid(15.0*0 + (-7.0)) = sigmoid(-7.0) ≈ 0.001
        # 다를 때 (difference≈2): sigmoid(15.0*2 + (-7.0)) = sigmoid(23.0) ≈ 1.000
        gate_input = self.alpha * difference + self.bias_offset
