...
class AuxiliaryHeadFusionV2_6(nn.Module):
    """
    🔥 Auxiliary Head Pretrain 전략 기반 융합 모듈 (v2_6)
    ...
    3. ✅ Confidence 기반 Fusion: freeze 후에도 confidence score를 계속 뽑아서 사용
    ...
    - **Phase 2 (Epoch 5+)**: Frozen auxiliary head + dynamic fusion
      * Auxiliary head 가중치 고정 (no gradient)
      * Confidence 기반 동적 가중치 사용
      * Auxiliary head는 inference만 수행 (confidence 추출용)
    
    학습 목표:
    1. 각 task마다 pretrain으로 안정적인 auxiliary head 학습
    2. Confidence score를 통한 모달리티별 신뢰도 측정
    3. 동적 가중치로 최적의 fusion
    """
    ...
    def set_epoch(self, epoch):
        ...
        if epoch == self.pretrain_epochs and not self.auxiliary_heads_frozen:
            ...
            if epoch not in self.epoch_logged:
                ...
                logging.info(f"   Phase Transition: Pretrain → Dynamic Fusion")
                logging.info(f"   Weight Mode: Uniform (1:1:1) → Confidence-based")
                logging.info(f"   Auxiliary Heads: Training → FROZEN (inference only)")
                logging.info(f"   From now on: Using frozen heads for confidence estimation")
                logging.info(f"{'='*70}")
    ...
    def forward(self, features, targets=None):
        """
        Forward pass: Pretrain + Dynamic Fusion (모든 task에 적용)
        ...
        Phase 2 (Dynamic Fusion, epoch 5+ in each task):
        - Auxiliary heads frozen (inference only)
        - Confidence 기반 동적 가중치 fusion
        ...
        if self.first_forward_per_task.get(self.current_task_id, False):
            ...
            logging.info(f"   🔧 Architecture: Pretrain + Dynamic Fusion Strategy")
            ...
            logging.info(f"   🔒 Current Phase: {'Pretrain' if is_pretrain else 'Frozen Inference'}")
        ...
        return {
            ...
            'fusion_type': 'auxiliary_head_v2_6',
