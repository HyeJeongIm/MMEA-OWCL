...
class IMUEuclideanGateFusion(nn.Module):
    ...
    def _compute_imu_weights(self, f_gyro, f_acce):
        ...
        핵심 로직 (반대 버전):
        - 공통 특징 공간으로 투영 → Euclidean distance 계산 → 거리 반전 → sigmoid로 게이트 강도 계산
        - 거리가 작을수록 (서로 유사할수록) 두 센서를 모두 강하게 활용
        ...
        # 🔄 반대 로직: 거리가 작을수록 (더 유사할수록) 강한 가중치
        # 기존 가정 뒤집기: 유사한 정보 = 신뢰도 높음 = 더 사용
        inverted_dist = 1.0 - normalized_dist  # 거리 반전: 가까울수록 큰 값
        gate_strength = torch.sigmoid(self.alpha * inverted_dist)  # [Batch]
